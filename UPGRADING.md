...
6. SYNCのdry-runとAUDITを一度ずつ実行し、結果を人間が確認する。

## 1.xから2.0への移行

- `~/.codex/prompts/` の旧プロンプトは、`.agents/skills/` 版の動作確認後に削除する。
- `STATE/README.md` 冒頭へ `last_sync_at`、`last_sync_commit`、
  `activity_log_cursors` のYAMLを追加する。旧 `experiment_log_cursors` はインストーラが名称を移行する。
